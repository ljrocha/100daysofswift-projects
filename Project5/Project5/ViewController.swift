...
//  Project5
//
//  Created by Leandro Rocha on 3/23/19.
...
class ViewController: UITableViewController {
        ...
        if isPossible(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                if isReal(word: lowerAnswer) {
                    ...
                    showErrorMessage(title: "Word not recognized", message: "Word must be real and at least 3 characters in length!")
                }
            } else {
                showErrorMessage(title: "Word used already", message: "Choose a word that hasn't been used and is not the start word!")
            }
        } else {
    ...
    func isOriginal(word: String) -> Bool {
        guard let title = title, title != word else { return false }
